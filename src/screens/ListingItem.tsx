...
import { Firestore, Timestamp, collection, addDoc, doc, getDocs, getFirestore, query, runTransaction, setDoc, where } from 'firebase/firestore';
                           ...
                           <View style={[styles.productInformation, {width: scaleIt(237)}]}>
                               ...
                               <View style={{ marginTop: 10 }}>
                                   ...
                                   <Pressable onPress={(currentUser?.uid != item.userId) && (async () => {
                                       ...
                                       const q = query(chatsRef, where("users", "array-contains", (currentUser?.uid + item.userId)));
                                       const querySnapshot = await getDocs(q);

                                       if(querySnapshot.empty) {
                                           try {
                                            //    const chatsRef = doc(collection(db, 'chats'));
                                               await setDoc(doc(db, "chats", currentUserId + item.userId), { id: currentUserId + item.userId, users: [currentUserId+item.userId, item.userId + currentUserId], messages: [{message: "Hey, I'm interested in your listing!", id: Math.random().toString(36).substring(7), sender_id: currentUserId, receiver_id: item.userId, timestamp: new Date()}, {message: "Hey, thanks for messaging me!", id: Math.random().toString(36).substring(7), sender_id: item.userId, receiver_id: currentUserId, timestamp: new Date()}], user1_id: currentUserId, user2_id: item.userId });
